...
bool loadAudioFile(const char* filename) {
  ...
  if (channels == 1) {
    ...
    if (read != info.frames) {
      std::cerr << "Warning: read " << read << " of " << info.frames
                << " frames" << std::endl;
    }
  } else {
...
void configurePlayback(float sr) {
  ...
  g_xplay.setFadeTime(0.5f);
  g_xplay.setGate(1.0f);
  g_xplay.setRate(0.5f);
  //   g_xplay.setPlayMode(XPlay::PlayMode::Bounce);
  g_xplay.setStartEnd(0.25, 0.75);
...
int main() {
  ...
  std::cout << "================================" << std::endl;
  std::cout << "Mouse X -> start (0..1), Mouse Y -> end (0..1) with 200ms Lag "
               "smoothing."
            << std::endl;
  ...
  while (g_running.load()) {
    ...
    if (XQueryPointer(display, root, &returnedRoot, &returnedChild, &rootX,
      ...
      float normY =
          static_cast<float>(rootY) / static_cast<float>(screenHeight);
